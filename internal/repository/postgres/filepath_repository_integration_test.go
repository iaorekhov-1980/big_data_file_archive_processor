...
func TestFilePathRepository_Integration(t *testing.T) {
	...
	testFile := createTestFile("testfilehash", "testfile.txt")
	...
	t.Run("InsertFilePath and GetFilePathByPathAndSource", func(t *testing.T) {
		...
		filePath := createTestFilePath("/path/to/file.txt", "source1", "testfilehash", true)
	...
	t.Run("InsertFilePath_Duplicate", func(t *testing.T) {
		...
		path1 := createTestFilePath("/duplicate/path", "source1", "testfilehash", true)
		...
		path2 := createTestFilePath("/duplicate/path", "source1", "testfilehash", false)
	...
	t.Run("InsertFilePath_SamePathDifferentSources", func(t *testing.T) {
		...
		path1 := createTestFilePath("/same/path", "source1", "testfilehash", true)
		err := repo.InsertFilePath(ctx, path1)
		require.NoError(t, err, "Failed to insert file path for source1")
		
		path2 := createTestFilePath("/same/path", "source2", "testfilehash", true)
		err = repo.InsertFilePath(ctx, path2)
	...
	t.Run("UpdateFilePathDeletedAtByPathAndSource", func(t *testing.T) {
		// Insert a file path
		filePath := createTestFilePath("/path/to/delete.txt", "source1", "testfilehash", true)
		err := repo.InsertFilePath(ctx, filePath)
		require.NoError(t, err, "Failed to insert file path")
	...
	t.Run("GetFilePathsBySourceAndActive", func(t *testing.T) {
		...
		for _, p := range paths {
			fp := createTestFilePath(p.path, "test-source", "testfilehash", p.isActive)
	...
	t.Run("GetFilePathsBySourceAndInactiveNotDeleted", func(t *testing.T) {
		...
		fp1 := createTestFilePath("/inactive/notdeleted", "test-source", "testfilehash", false)
		err = repo.InsertFilePath(ctx, fp1)
		require.NoError(t, err)
		
		// Path 2: Inactive, deleted
		fp2 := createTestFilePath("/inactive/deleted", "test-source", "testfilehash", false)
		fp2.DeletedAt = &now
		err = repo.InsertFilePath(ctx, fp2)
		require.NoError(t, err)
		
		// Path 3: Active, not deleted
		fp3 := createTestFilePath("/active/notdeleted", "test-source", "testfilehash", true)
