...
func TestPostgresRepository_Integration(t *testing.T) {
	...
	config := testutils.GetTestDBConfig(t)
	...
	t.Run("FullRepositoryLifecycle", func(t *testing.T) {
		// Clean up first
		testutils.CleanTestData(ctx, repo.pool)

		// Step 1: Create a file
		file := testutils.CreateTestFile("integration-hash", "integration-file.txt")
		...
		filePath := testutils.CreateTestFilePath("/integration/path.txt", "integration-source", "integration-hash", true)
		...
		sourceProcessing := testutils.CreateTestSourceProcessing("integration-source")
	...
	t.Run("RepositoryCompositionWorks", func(t *testing.T) {
		...
		file := testutils.CreateTestFile("compose-hash", "compose-file.txt")
		err := repo.InsertFile(ctx, file)
		require.NoError(t, err)
		...
		filePath := testutils.CreateTestFilePath("/compose/path.txt", "compose-source", "compose-hash", true)
		err = repo.InsertFilePath(ctx, filePath)
		require.NoError(t, err)
		...
		sourceProcessing := testutils.CreateTestSourceProcessing("compose-source")
		err = repo.UpsertSourceProcessing(ctx, sourceProcessing)
		require.NoError(t, err)
	...
	t.Run("TransactionIsolation", func(t *testing.T) {
		...
		// Insert a file
		file1 := testutils.CreateTestFile("tx-hash1", "tx-file1.txt")
		err := repo.InsertFile(ctx, file1)
		require.NoError(t, err)

		// Insert another file
		file2 := testutils.CreateTestFile("tx-hash2", "tx-file2.txt")
		...
		file3 := testutils.CreateTestFile("tx-hash1", "tx-file3.txt")
